...
from llm_planner_alfred.hlp_planner import LLM_Planner
...
class AlfredEvaluator:
    def __init__(self, config_file):
        ...
        self.obj_encoder = SentenceTransformer(
            self.config["llm_planner"]["emb_model_name"]
        )
        self.obj_sim_threshold = self.config["llm_planner"].get(
    ...
    def _prepare_tasks(self):
        ...
        if self.config.get("debug", False):
            for task in tasks:
                if (
                    "trial_T20190906_201106_979461" in task["task"]
                ):  # NOTE Change this to the task you want to debug
                    new_task = [task]
                    break
            tasks = new_task
    ...
    def evaluate_task(
        ...
        self.env.set_task(
            traj_data, dotdict(self.config["alfred"]["env_args"]), reward_type="dense"
        )
        ...
        step_instrs = [
            ann["high_descs"] for ann in traj_data["turk_annotations"]["anns"]
        ]

        # Get configuration for using step instructions
        use_step_instructions = self.config["llm_planner"].get(
        ...
        formatted_task = {
            ...
            "step_instr": (
                [step for sublist in curr_task["step_instr"] for step in sublist]
                if curr_task["step_instr"]
                and isinstance(curr_task["step_instr"][0], list)
                else curr_task["step_instr"]
            ),
        ...
        while high_level_plans and replanning_count <= max_replanning:
            ...
            try:
                ...
                if "instruction not supported" in str(e).lower():
                    log.warning(
                        f"Instruction '{plan}' not supported. Skipping and continuing."
                    )
                    ...
                    if matched_obj:
                        ...
                        log.debug(
                            f"Object '{object_name}' not found. Using fuzzy match '{matched_obj}' (similarity: {similarity:.2f})"
                        )
                        ...
                        try:
                            ...
                            if action_ret["success"]:
                                log.debug(
                                    f"SUCCESS: '{matched_plan}' executed successfully (fuzzy match)"
                                )
                            ...
                            log.warning(
                                f"Error executing fuzzy matched plan: {str(new_e)}"
                            )
                    else:
                        log.warning(
                            f"No matching object found for '{object_name}'. Best similarity: {similarity:.2f} (threshold: {self.obj_sim_threshold})"
                        )
                else:
            ...
            if not action_ret["success"]:
                ...
                if (
                    "instruction not supported" in action_ret["message"].lower()
                    or "not supported" in action_ret["message"].lower()
                ):
                    log.warning(
                        f"Instruction '{plan}' not supported. Skipping and continuing."
                    )
                ...
                if (
                    "object" in action_ret["message"].lower()
                    and "not found" in action_ret["message"].lower()
                    and object_name
                ):
                    # Try fuzzy matching if object not found
                    available_objects = self.env.get_visible_objects().split(", ")
                ...
                if dynamic:
                    ...
                    if retry_count >= max_retries:
                        log.warning(
                            f"Exceeded maximum retries ({max_retries}) for this action. Moving on."
                        )
                        retry_count = 0
                        continue
                    ...
                    if replanning_count >= max_replanning:
                        log.warning(
                            f"Exceeded maximum replanning attempts ({max_replanning}) for this task. Continuing with remaining plans."
                        )
                        retry_count = 0
                        continue
                    ...
                    replanning_count += 1
                    log.debug(
                        f"Dynamic replanning attempt {retry_count}/{max_retries} for current action (Total: {replanning_count}/{max_replanning})"
                    )
                    ...
                    formatted_task = {
                        ...
                        "step_instr": (
                            [
                                step
                                for sublist in curr_task["step_instr"]
                                for step in sublist
                            ]
                            if curr_task["step_instr"]
                            and isinstance(curr_task["step_instr"][0], list)
                    ...
                    if use_step_instructions:
                        log.debug(
                            f"  Step instructions: {formatted_task['step_instr']}"
                        )
                    else:
                        log.debug("  Step instructions: Disabled")
                    log.debug(f"  Visible objects: {formatted_task['vis_objs']}")
                    ...
                    if vision:
                        ...
                        encoded_frames = [
                            self.encode_image(frame) for frame in curr_frame
                        ]

                    # Generate new plans for dynamic replanning
                    llm_out = self.llm(
                        new_prompt, engine=engine, images=encoded_frames, stop=["\n"]
                    )
                    high_level_plans = self.clean_llm_output(llm_out)
        ...
        if replanning_count >= max_replanning:
            log.warning(
                f"Task ended because maximum replanning attempts ({max_replanning}) were reached."
            )
    ...
    def run_evaluation(self, dry_run=False):
        ...
        # Main eval loop
        start = time.time()
        for task_idx, task in tqdm(enumerate(self.tasks), desc="Tasks"):
            try:
                log.debug(task)
                traj_data = load_task_json(task)
                r_idx = task["repeat_idx"]
                log.debug(
                    f"Evaluating ({task_idx+1}/{len(self.tasks)}): {traj_data['root']}"
                )
                ...
                traceback.print_exc()
                log.error(
                    f"Error processing task {task_idx+1}/{len(self.tasks)}: {repr(e)}"
                )
    ...
    def _print_results(self, results, start_time):
        ...
        if n == 0:
            log.warning(
                "No results collected - all tasks may have failed with exceptions"
            )
            log.debug(f"success rate: 0.00 % (0/0)")
            log.debug(
                f"elapsed: {str(datetime.timedelta(seconds=(time.time() - start_time)))}"
            )
            log.debug("------------------------")
    ...
    def save_result(self, result_dict, base_path):
        ...
        task_dir = os.path.join(
            base_path, f"{result_dict['trial']}_{result_dict['repeat_idx']}"
        )
        ...
        with open(os.path.join(task_dir, "plans.txt"), "w") as f:
            ...
            # Final result
            f.write(
                f"Final Status: {'SUCCESS' if result_dict['success'] else 'FAILURE'}"
            )
    ...
    def match_object_name(self, generated_name, available_objects):
        ...
        try:
            ...
            generated_embedding = self.obj_encoder.encode(
                generated_name, convert_to_tensor=True, show_progress_bar=False
            )

            # Encode all available objects
            available_embeddings = self.obj_encoder.encode(
                available_objects, convert_to_tensor=True, show_progress_bar=False
            )
...
def main():
    ...
    if evaluator.config.get("save_results", False):
        ...
        result_file = os.path.join(
            evaluator.config["out_dir"], f"results_{timestamp}.json"
        )
