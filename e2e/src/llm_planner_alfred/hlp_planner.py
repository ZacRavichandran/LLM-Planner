...
# Example to get the path to data.json
def get_data_path():
    resource_package = "llm_planner_alfred.resources"
    resource_name = "knn_set.pkl"
...
def get_real_file_path():
    resource = files("llm_planner_alfred.resources") / "knn_set.pkl"
    with as_file(resource) as file_path:
        print(file_path)  # This is a pathlib.Path object you can safely use
        return str(file_path)


KNN_DATA_PATH = get_real_file_path()


def clean_llm_output(llm_out):
    """
...
class LLM_Planner:
    ...
    def knn_retrieval(self, curr_task, k):
        ...
        traj_emb = self.sentence_embedder.encode(
            curr_task["task_instr"], show_progress_bar=False
        )
        topK = []
        for idxTrain, trainItem in self.knn_set.iterrows():

            train_emb = self.sentence_embedder.encode(
                trainItem["task_instr"], show_progress_bar=False
            )
    ...
    def generate_prompt(
        ...
        log.debug(
            f"Generating prompt with {k} examples, includeLow={includeLow}, dynamic={dynamic}"
        )
        ...
        for retrieved_task in knn_retrieved_examples:
            ...
            step_list = [
                literal_eval(listItem)
                for rowItem in trainTaskRow["gold_traj"]
                for listItem in rowItem
            ]
